  ...
  try {
    ...
    for (const req of requests) {
      if (Array.isArray(req.facultySnapshot)) {
        req.facultySnapshot = req.facultySnapshot.filter(f => f.subjectId?.toString() !== subjectId.toString());
...
/**
 * Synchronize update of a subject (faculty change) with active batches.
 */
...
export const syncMentorUpdate = async (studentId, mentorId, mentorName) => {
  ...
  try {
    ...
    const activeBatch = await NodueBatch.findOne({ _id: request.batchId, status: 'active' }).session(session).lean();
    if (!activeBatch) {
        await session.abortTransaction();
        return;
    }

    // 1. Update facultySnapshot
    if (Array.isArray(request.facultySnapshot)) {
        const idx = request.facultySnapshot.findIndex(f => f.roleTag === 'mentor');
        if (idx !== -1) {
            request.facultySnapshot[idx] = { ...request.facultySnapshot[idx], facultyId: mentorId, facultyName };
        } else {
            request.facultySnapshot.push({
                facultyId: mentorId, facultyName, roleTag: 'mentor', approvalType: 'mentor',
        ...
        request.facultySnapshot['mentor'] = {
            facultyId: mentorId, facultyName, roleTag: 'mentor', approvalType: 'mentor',
            subjectId: null, subjectName: 'Institutional Mentor'
        };
...
/**
 * Synchronize elective removal for a student.
 */
export const syncElectiveRemoval = async (studentId, subjectId) => {
    ...
    try {
        ...
        if (Array.isArray(request.facultySnapshot)) {
            request.facultySnapshot = request.facultySnapshot.filter(f => f.subjectId?.toString() !== subjectId.toString());
        } else if (request.facultySnapshot) {
...
export const syncStudentDeactivation = async (studentId) => {
    ...
    try {
        ...
        // 1. Delete all approval records for this student in this batch
...
/**
 * Synchronize bulk mentor assignment.
 */
export const bulkSyncMentorUpdate = async (studentIds, mentorId, mentorName) => {
    ...
    try {
        ...
        // Find all requests for these students in active batches
        ...
        const activeBatches = await NodueBatch.find({ _id: { $in: batchIds }, status: 'active' }).session(session).select('_id');
            ...
            if (Array.isArray(req.facultySnapshot)) {
                ...
                if (idx !== -1) {
                    ...
                    req.facultySnapshot.push({
                        ...
                        approvalType: 'mentor', subjectName: 'Institutional Mentor'
                    });
                }
            } else if (req.facultySnapshot) {
                req.facultySnapshot['mentor'] = {
                    facultyId: mentorId, facultyName: mentorName, roleTag: 'mentor', 
                    approvalType: 'mentor', subjectId: null, subjectName: 'Institutional Mentor'
                };
                req.markModified('facultySnapshot');
            }
            await req.save({ session });

            // Update/Upsert approval
            await NodueApproval.findOneAndUpdate(
...
/**
 * Synchronize bulk student deactivation.
 */
...
export const syncClassChange = async (studentId, oldClassId, newClassId) => {
    ...
    try {
        ...
        // 1. Deactivate old request
        // We reuse the logic but pass the session
        ...
        const activeBatch = await NodueBatch.findOne({ classId: newClassId, status: 'active' }).session(session).lean();
        if (!activeBatch) {
            await session.commitTransaction();
            return;
        }
        ...
        // Fetch required faculty
        const [hodAccount, ctInfo, mentor] = await Promise.all([
            Faculty.findOne({ departmentId: cls.departmentId._id, roleTags: 'hod', isActive: true }).session(session).select('name').lean(),
            cls.classTeacherId ? Faculty.findById(cls.classTeacherId).session(session).select('name').lean() : null,
            student.mentorId ? Faculty.findById(student.mentorId).session(session).select('name').lean() : null
        ]);
        ...
        for (const s of cls.subjectAssignments ?? []) {
            ...
            snapshot[s.subjectId.toString()] = {
                facultyId: s.facultyId, facultyName: s.facultyName, subjectId: s.subjectId,
                subjectName: s.subjectName, subjectCode: s.subjectCode, roleTag: 'faculty', approvalType: 'subject'
            };
        }

        if (cls.classTeacherId) {
            snapshot['classTeacher'] = {
                facultyId: cls.classTeacherId, facultyName: ctInfo?.name ?? null, roleTag: 'classTeacher',
                approvalType: 'classTeacher', subjectName: 'Academic Advisor (Class Teacher)'
            };
        }

        if (student.mentorId) {
            snapshot['mentor'] = {
                facultyId: student.mentorId, facultyName: mentor?.name ?? null, roleTag: 'mentor',
                approvalType: 'mentor', subjectName: 'Institutional Mentor'
            };
        }
        ...
        for (const e of student.electiveSubjects ?? []) {
            ...
            snapshot[e.subjectId.toString()] = {
                facultyId: e.facultyId, facultyName: e.facultyName, subjectId: e.subjectId,
                subjectName: e.subjectName, subjectCode: e.subjectCode, roleTag: 'faculty', approvalType: 'subject'
            };
        }
...
/**
 * Synchronize addition of a subject to a class with active batches.
 */
export const syncSubjectAddition = async (classId, subjectData) => {
    ...
    try {
        ...
        // 1. Add to facultySnapshot of all requests in this batch
        const requests = await NodueRequest.find({ batchId: activeBatch._id }).session(session);
        for (const req of requests) {
            if (Array.isArray(req.facultySnapshot)) {
                req.facultySnapshot.push({
        ...
        // 2. Create approval records for all students in the batch
