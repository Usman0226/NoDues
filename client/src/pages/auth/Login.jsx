    ...
    <div className="min-h-screen flex items-center justify-center bg-offwhite p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-10">
          <h1 className="text-5xl font-brand text-navy mb-2">No<span className="text-gold">Dues</span></h1>
          <p className="tagline text-muted-foreground uppercase tracking-[0.2em] font-bold text-[10px]">MITS Academic Clearance Platform</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-muted overflow-hidden">
          ...
          <div className="flex border-b border-muted">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-4 text-[10px] font-black uppercase tracking-[0.15em] transition-all
                ${activeTab === 'staff'
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-4 text-[10px] font-black uppercase tracking-[0.15em] transition-all
                ${activeTab === 'student'
                  ...
                  : 'text-muted-foreground hover:text-navy bg-offwhite/50'}`}
            >
              <UserIcon size={14} /> Student Login
            </button>
          </div>

          <div className="p-8">
            {error && (
              <div className="mb-4 p-3 rounded-xl bg-red-50 text-red-600 text-[10px] font-black uppercase tracking-widest border border-red-100">{error}</div>
            )}
            ...
            {activeTab === 'staff' && (
              <form onSubmit={handleStaffLogin} className="space-y-4">
                ...
                <div className="pt-2">
                  <Button type="submit" className="w-full h-12 text-[10px] font-black uppercase tracking-[0.2em]" loading={loading}>Authorize Access</Button>
                </div>
              </form>
            )}
            ...
            {activeTab === 'student' && (
              <form onSubmit={handleStudentLogin} className="space-y-5">
                ...
                <Button type="submit" className="w-full h-12 text-[10px] font-black uppercase tracking-[0.2em]" loading={loading}>Retrieve Status</Button>
              </form>
            )}
            ...
            <div className="mt-10 pt-8 border-t border-muted/50">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                <button onClick={() => fillDemo('admin')} className="px-3 py-1.5 rounded-lg bg-navy/5 text-navy text-[9px] font-black uppercase tracking-widest hover:bg-navy hover:text-white transition-all border border-navy/10">Admin</button>
                <button onClick={() => fillDemo('hod')} className="px-3 py-1.5 rounded-lg bg-navy/5 text-navy text-[9px] font-black uppercase tracking-widest hover:bg-navy hover:text-white transition-all border border-navy/10">HOD</button>
                <button onClick={() => fillDemo('faculty')} className="px-3 py-1.5 rounded-lg bg-navy/5 text-navy text-[9px] font-black uppercase tracking-widest hover:bg-navy hover:text-white transition-all border border-navy/10">Faculty</button>
                <button onClick={() => fillDemo('student')} className="px-3 py-1.5 rounded-lg bg-navy/5 text-navy text-[9px] font-black uppercase tracking-widest hover:bg-navy hover:text-white transition-all border border-navy/10">Student</button>
              </div>
            </div>
          </div>
        </div>
